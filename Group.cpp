...
void Group::Add_Student() // старый вариант добавления студента
{
	Student *new_student = new Student[count_of_student+1];
	for (int i = 0; i < count_of_student; i++)
	{
...
void Group::Add_Student(const char* surname, const char* name, const char* patronymic)
{
	Student* new_student = new Student  [count_of_student + 1];
	for (int i = 0; i < count_of_student; i++)
	{
	...
	new_student[count_of_student] = Student(surname, name, patronymic);
